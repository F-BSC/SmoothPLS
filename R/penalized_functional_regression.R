...
#### ETAPE 2 : L'ASSEMBLEUR GLOBAL (The Matrix Assembler) ####
...
#'
#' @description
#' Constructs the block-diagonal penalty matrix, multiplies each block by its
#' specific lambda, and adds zero-padding for the intercept.
#'
#' @param block_list A list of blocks generated by the build_block_* functions.
#' @param lambda_vec A numeric vector of lambdas, one for each predictor block.
#'
#' @return A symmetric square matrix representing R_0_lambda
build_global_penalty <- function(block_list, lambda_vec) {

  if (length(block_list) != length(lambda_vec)) {
    stop("lambda_vec must have exactly one penalty value per predictor block.")
  }

  penalties_to_diag <- list()

  # 1. Préparer les blocs pénalisés
  for (i in seq_along(block_list)) {
    block <- block_list[[i]]
    lambda <- lambda_vec[i]

    if (block$type == "cfd") {
      # Pour un CFD, on a K_prime états. On duplique la matrice R_base K_prime fois
      # (Pour cette version, on applique le même lambda à tous les états d'un CFD)
      R_penalized <- lambda * block$R
      cfd_blocks <- replicate(block$K_prime, R_penalized, simplify = FALSE)

      # On fusionne ces K_prime blocs en un sous-bloc diagonal
      penalties_to_diag[[i]] <- bdiag_base(cfd_blocks)

    } else {
      # Pour NFD et SFD, c'est direct : lambda * R
      penalties_to_diag[[i]] <- lambda * block$R
    }
  }
  ...
  # (On réutilise ici ta fonction augment_penalty_matrix déjà codée !)
