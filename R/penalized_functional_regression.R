...
cv_pfr_multi_no_parallel <- function(block_list, Y, list_of_lambda_lists) {
...
#' Multivariate Exact LOOCV for PFR (Parallelized)
#'
#' @param block_list A list of blocks generated by the build_block_* functions.
#' @param Y The response vector.
#' @param list_of_lambda_lists A list containing all the lambda combinations to test.
#' @param parallel Logical. If TRUE, executes the grid search in parallel.
#'
#' @return A list with the optimal lambda combination and the minimum RMSEP.
#'
#' @importFrom future.apply future_lapply
cv_pfr_multi <- function(block_list, Y, list_of_lambda_lists, parallel = FALSE) {

  # 1. Compute the global design matrix ONCE. It is invariant to lambda.
  D_global <- build_global_design(block_list, length(Y))
  n_models <- length(list_of_lambda_lists)

  # 2. Define a worker function to evaluate a single lambda combination
  evaluate_single_combination <- function(current_lambda_list) {
    # Build dynamic penalty matrix
    R_0_lam <- build_global_penalty(block_list, current_lambda_list)

    # Fit model
    fit <- fit_pfr_multi(D_global, Y, R_0_lam)

    # Handle singular matrices safely
    if (is.null(fit)) return(Inf)

    # Fast Exact LOOCV residuals: e_i / (1 - h_ii)
    res_loo <- fit$residuals / (1 - fit$h_diag)
    return(sqrt(mean(res_loo^2)))
  }

  # 3. Dispatch computation based on parallel flag
  if (parallel) {
    # Parallel execution via future.apply
    # future.seed = TRUE ensures reproducibility if any random state exists
    rmsep_vec <- unlist(future.apply::future_lapply(
      X = list_of_lambda_lists,
      FUN = evaluate_single_combination,
      future.seed = TRUE
    ))
  } else {
    # Standard sequential execution
    rmsep_vec <- unlist(lapply(
      X = list_of_lambda_lists,
      FUN = evaluate_single_combination
    ))
  }
...
mpfr <- function(df_list, Y,
  ...
  # If the user did not provide a grid, compute it automatically via trace heuristics
  ...
  if (parallel) {
    if (print_steps) cat("=> Setting up parallel backend (multisession)...\n")

    # Save the user's current plan and ensure it's restored on exit (CRAN requirement)
    old_plan <- future::plan()
    on.exit(future::plan(old_plan), add = TRUE)

    # Activate multi-core processing (works on Windows, Mac, Linux)
    future::plan(future::multisession)
  }

  if (print_steps) cat("=> Running Exact LOOCV Optimization...\n")
  # Pass the parallel flag down to the CV engine
  cv_results <- cv_pfr_multi(block_list = block_list,
                             Y = Y,
                             list_of_lambda_lists = list_of_lambda_lists,
                             parallel = parallel)

  # best hyper parameters gathering
